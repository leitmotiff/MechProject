...
public class Move1 : MonoBehaviour
{
    ...
    private void Move() {
		...
		if (rx > Screen.width*0.8f)
			rxnorm = rx/Screen.width;
		if (rx < Screen.width*0.2f)
			rxnorm = (Screen.width*.2f - rx)/(Screen.width*.2f) * -1;
		
		if (ry > Screen.height*0.7f)
			rynorm = ry/Screen.height;
		if (ry < Screen.height*0.3f)
			rynorm = (Screen.height*.3f - ry)/(Screen.height*.3f) * -1;
		if(!invertYAxis)
			rynorm*=-1;
